...
export async function POST(request: Request) {
  ...
  try {
    ...
    const action = await adapter.buildTransferToSpot(amount, { wallet });

    return NextResponse.json({
      success: true,
      action,
      message: `Sign transaction to withdraw ${amount} USDC from perps to spot`,
    });
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err);
    return NextResponse.json({ error: `Withdraw failed: ${message}` }, { status: 500 });
  }
}
