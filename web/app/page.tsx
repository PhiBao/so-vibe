...
function StatBlock({ label, value, sub, color = "cyan" }: { label: string; value: string; sub?: string; color?: "cyan" | "green" | "red" | "magenta" | "yellow" }) {
  ...
  return (
    <div className="terminal-card p-4 animate-in">
      ...
      {sub && <div className="text-[10px] text-[var(--text-secondary)] mt-1 font-mono">{sub}</div>}
    </div>
  );
}
...
function MarketRow({ sym, data }: { sym: string; data: Record<string, unknown> }) {
  ...
  return (
    <div className="flex items-center justify-between py-3 border-b border-[var(--border)] last:border-0 hover:bg-white/[0.02] px-2 transition-colors">
      <div className="flex items-center gap-3 w-32"><span className="text-[var(--cyan)] text-sm font-bold">{sym}</span><span className="text-[10px] text-[var(--text-secondary)]">-PERP</span></div>
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        let symbols = ["SOL-USD", "ETH-USD", "BTC-USD"];
        try {
          const marketsRes = await fetch("/api/markets");
          const marketsData = await marketsRes.json();
          if (marketsData.markets) {
            const fetched = Object.keys(marketsData.markets).sort();
            if (fetched.length > 0) {
              symbols = fetched;
    ...
    <div className="max-w-6xl mx-auto space-y-6 animate-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] text-[var(--text-secondary)] font-mono mt-1">Agentic swarm trading on SoDEX perpetuals</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 terminal-card text-[10px]">
            ...
            {botStatus.running && <span className="text-[var(--text-secondary)]">cycle #{botStatus.cycle}</span>}
          </div>
        </div>
      </div>
        ...
        <>
          ...
          <div className="terminal-card border-l-2 border-l-[var(--cyan)] sticky top-4 z-10">
            <div className="terminal-header">
              ...
              <span className="text-[10px] text-[var(--text-secondary)] ml-auto">{address?.slice(0, 6)}...{address?.slice(-4)}</span>
            </div>
            <div className="p-4 space-y-4">
              {/* SoDEX Account ID */}
              <div className="flex items-center gap-2">
                <label className="text-[10px] text-[var(--text-secondary)] uppercase tracking-[0.15em]">SoDEX Account ID</label>
              ...
              {walletPositions.length > 0 ? (
                ...
                <div className="text-center py-6 text-[var(--text-secondary)] text-[12px] font-mono">No open positions on SoDEX</div>
              )}
            </div>
          </div>
        </>
      ...
      <div className="terminal-card">
        ...
        <div className="p-4">
          <div className="flex items-center justify-between text-[10px] text-[var(--text-secondary)] uppercase tracking-wider px-2 pb-2 border-b border-[var(--border)] mb-2">
          ...
          {symbolList.map((sym) => (<MarketRow key={sym} sym={sym} data={markets[sym] || {}} />))}
        </div>
      </div>
      ...
      {isConnected && pendingSignals > 0 && (
        <div className="terminal-card border-l-2 border-l-[var(--yellow)]">
          <div className="terminal-header">
            ...
            <span className="text-[10px] text-[var(--text-secondary)] ml-auto">{pendingSignals} queued</span>
          </div>
      ...
      {positions.length > 0 && (
        <div className="terminal-card">
          <div className="terminal-header"><span className="text-[11px] font-bold tracking-wider">BOT_POSITIONS</span><span className="text-[10px] text-[var(--text-secondary)] ml-auto">{positions.length} active</span></div>
          <div className="p-4 space-y-2">
            {positions.map((pos, i) => (
              <div key={i} className="flex items-center justify-between py-2 px-3 bg-white/[0.02] border border-[var(--border)]">
                ...
                <div className="text-[11px] text-[var(--text-secondary)] font-mono">SL: <span className="text-[var(--red)]">${(pos.stopLoss as number || 0).toFixed(2)}</span><span className="mx-2 text-[var(--text-secondary)]">|</span>TP: <span className="text-[var(--green)]">${(pos.takeProfit as number || 0).toFixed(2)}</span></div>
              </div>
            ))}
          </div>
        </div>
